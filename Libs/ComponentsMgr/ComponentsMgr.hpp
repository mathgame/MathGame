...
bool ComponentMgr::AddXmlNode(const pugi::xml_node& xmlNode, const std::string& hint)
{
    ...
    {
        COMP_LOG << "Unknown xml node: " << xmlNode.name();
        return false;
    }
    return true;
}
...
bool ComponentMgr::AddComponent(const pugi::xml_node& xmlNode, const std::string& hint)
{
    ...
    if (nullptr == comp)
    {
        COMP_LOG << "Error creating component from xml node: " << xmlNode.name();
        return false;
    }
    ...
    COMP_LOG << "Duplicate component: " << comp->GetComponentName() << ", hint : " << comp->GetComponentHint();
    return false;
}
...
{
    m_resolver = &(ComponentFactoryType::Resolve);
    m_combiner = &(ComponentFactoryType::Combine);
...
bool ComponentMgr::LoadFromXmlNode(const pugi::xml_node& xmlNode, std::string hint)
{
    ...
    try
    {
        if (false == IsManager(xmlNode))
        {
            ...
            COMP_LOG << "Invalid Component Manager Node : " << name;
            return false;
        }
        ...
        for (const auto& childNode : xmlNode.children())
        {
            if (false == AddXmlNode<ComponentFactoryType>(childNode, hint))
            {
                ...
                COMP_LOG << "Invalid Component Node in " << name << " -> ";
                return false;
            }
        }
    }
...
bool ComponentMgr::AddIncludeFile(const pugi::xml_node& xmlNode)
{
    ...
    {
        COMP_LOG << "Not set " << s_strXmlAttrFileName << " attribute.";
        COMP_ASSERT("");
        return false;
    }
    ...
    pugi::xml_parse_result result = spxmlDoc.load_file ( strFile.c_str(), pugi::parse_default | pugi::parse_pi );
    if ( !result )
    {
        COMP_LOG << "XML Error description: " << result.description();
        COMP_LOG << "XML Error offset: " << result.offset << "]\n";
        COMP_LOG << "File : " << strFile;
        COMP_ASSERT("");
        return false;
    }
    ...
    for (const auto it : spxmlDoc.select_nodes (query.c_str()))
    {
        for (const auto node : it.node().select_nodes("*"))
        {
            ...
            if (false == AddXmlNode<ComponentFactoryType>(incNode, strFile))
            {
                ...
                COMP_LOG << "Invalid Component Node in file : " << strFile;
                COMP_ASSERT("");
                return false;
            }
        }
...
ComponentMgrPtr ComponentMgr::CreateFromFile (const std::string& fileName, const std::string& mgrName)
{
    ...
    COMP_LOG << "Error creating from file : " << fileName << " ; mgrName : " << mgrName;
...
{
    pugi::xml_document spxmlDoc;
    pugi::xml_parse_result result = spxmlDoc.load_file ( fileName.c_str(), pugi::parse_default | pugi::parse_pi );
    if ( !result )
    {
        COMP_LOG << "XML Error description: " << result.description();
        COMP_LOG << "XML Error offset: " << result.offset << "]\n";
        COMP_LOG << "File : " << fileName;
        return false;
    }
    ...
    if (mainNode.empty())
    {
        COMP_LOG << "Cant find start mgr node : " << fileName << " ; mgrName : " << m_name;
        COMP_ASSERT("");
        return false;
    }
    ...
    COMP_LOG << "Error parsing file : " << fileName << " ; mgrName : " << m_name;
...
ComponentMgrPtr ComponentMgr::CreateFromStream(iUniStream& inpStream)
{
    ...
    {
        COMP_LOG << "Cant extract Component Manager stream.";
        return nullptr;
    }
    ...
    {
        COMP_LOG << "Cant extract Component Manager name.";
        return nullptr;
    }
    ...
    {
        COMP_LOG << "Cant extract components stream : " << mgrName;
        return nullptr;
    }
    ...
    while(!componentsStream.IsEmpty())
    {
        ...
        if (nullptr == comp)
        {
            COMP_LOG << "Error parsing components in component manager : " << mgrName;
            return nullptr;
        }
        ...
        {
            COMP_LOG << "Duplicate component in manager : " << mgrName << " / " << comp->GetComponentName();
            return nullptr;
        }
    }
